...
class SpeedometerPainter extends CustomPainter {
  final double animatedNumber; // 动画数字
  final double totalNumber; // 总数
  final Color progressColor; // 进度条的颜色

  SpeedometerPainter(this.animatedNumber, this.totalNumber, this.progressColor);
  ...
  void paint(Canvas canvas, Size size) {
    ...
    double percent = animatedNumber / totalNumber; // 根据总数计算当前的百分比
    ...
    TextSpan span = TextSpan(
      ...
      text: '${animatedNumber.toStringAsFixed(0)}', // 显示当前数字
    );
    ...
    // 绘制三角形指示器
    _drawSharpTriangleIndicator(canvas, percent, size, startAngle, sweepAngle);
  }

  // 绘制锐角三角形指示器
  void _drawSharpTriangleIndicator(Canvas canvas, percent, Size size, double startAngle, double sweepAngle) {
  ...
  @override
  bool shouldRepaint(covariant SpeedometerPainter oldDelegate) => oldDelegate.animatedNumber != animatedNumber || oldDelegate.totalNumber != totalNumber;
}

// StatefulWidget，用于创建具有动画的进度条
class SpeedometerNumber extends StatefulWidget {
  final double currentNumber;
  final double totalNumber;
  final int size; // 进度条的尺寸
  final Color color; // 进度条的颜色

  SpeedometerNumber({required this.currentNumber, required this.totalNumber, required this.size, this.color = Colors.blue}); // 添加颜色参数，默认为蓝色

  @override
  _SpeedometerNumberState createState() => _SpeedometerNumberState();
}

class _SpeedometerNumberState extends State<SpeedometerNumber> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _animation = Tween<double>(begin: 0.0, end: widget.currentNumber).animate(CurvedAnimation(
      parent: _controller!,
  ...
  Widget build(BuildContext context) {
    return Center(
      child: SizedBox(
        ...
        child: CustomPaint(
          painter: SpeedometerPainter(_animation!.value, widget.totalNumber, widget.color), // 使用动画的当前值和颜色绘制
        ),
      ),
    );
  }
